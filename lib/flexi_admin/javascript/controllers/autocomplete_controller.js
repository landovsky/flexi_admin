...
export default class extends Controller {
  ...
  // Opening an empty field used to show nothing at all, so a user faced with a
  // long list had to guess a search term before seeing a single option. Focus
  // now asks the backend for its default suggestions.
  focus() {
  ...
  async _search(string) {
    ...
    // An app may mark one record as the one the user is working on, so the
    // backend can float it to the top of an otherwise arbitrary list.
    const priority = this.inputTarget.dataset.autocompletePriorityValue;
    if (priority) url.searchParams.set("ac_priority", priority);
